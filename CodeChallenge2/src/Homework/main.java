...
public class main {
    public static void main(String[] args)
    {
        ...
        for(int i = 0 ; i < studioDatabase.length ; i ++)
        {
            ...
            Film[] filme = studioDatabase[i].getFilme();
                for (int j=0;j< filme.length;j++)
                   {   // boolean flagFilm=false;
                       Actor[] actori = filme[j].getActori();
                       for(int k=0; k< actori.length;k++)
                       {
                           if(actori[k].getNume().equals("actor cu 2 oscaruri"))
                           {
                               System.out.println("Get the studio names in witch plays the actor with name “actor cu 2 oscaruri: ->");
                               System.out.println(studioDatabase[i].getNume());
                           }

                           if(actori[k].getVarsta()>50)
                           {
                               //flagFilm=true;
                               System.out.println("Get movie names in witch plays at least an actor with age above 50 : ->");
                               System.out.println(filme[j].getNume());
